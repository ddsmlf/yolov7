...
class YoloTrainer:
    def __init__(self, data_yaml = 'data/coco.yaml', name = 'exp', num_workers= 8, devices = '0', weights_path = 'yolo7.pt', batch_size = 16,
                 cfg = '', hyp = 'data/hyp.scratch.p5.yaml', epochs = 300, img_size = [640,640], rect = False, resume = False, nosave = False,
                 notest = False, noautoanchor = False, evolve = False, bucket = '', cache_images = False, image_weights = False,
                 multi_scale = False, single_cls = False, adam = False, sync_bn = False, local_rank = -1, project = 'runs/train', entity = None,
                 exist_ok = False, quad = False, linear_lr = False, label_smoothing = 0.0, upload_dataset = False, bbox_interval = -1,
    ...
    def train(self, hyp, device, tb_writer=None):
        ...
        if pretrained:
            ...
            model = Model(self.opt.cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
            ...
            self.logger.info('Transferred %g/%g items from %s' % (len(state_dict), len(model.state_dict()), weights))  # report
        else:
            model = Model(self.opt.cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        with torch_distributed_zero_first(rank):
        ...
        self.logger.info(f"Scaled weight_decay = {hyp['weight_decay']}")
        ...
        dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, self.opt,
                                                hyp=hyp, augment=True, cache=self.opt.cache_images, rect=self.opt.rect, rank=rank,
        ...
        assert mlc < nc, 'Label class %g exceeds nc=%g in %s. Possible class labels are 0-%g' % (mlc, nc, self.opt.data, nc - 1)

        # Process 0
        if rank in [-1, 0]:
            testloader = create_dataloader(test_path, imgsz_test, batch_size * 2, gs, self.opt,  # testloader
                                        hyp=hyp, cache=self.opt.cache_images and not self.opt.notest, rect=True, rank=-1,
                                        world_size=self.opt.world_size, workers=self.opt.workers,
        ...
        self.logger.info(f'Image sizes {imgsz} train, {imgsz_test} test\n'
        ...
        if rank in [-1, 0]:
            ...
            # Test best.pt
            self.logger.info('%g epochs completed in %.3f hours.\n' % (epoch - start_epoch + 1, (time.time() - t0) / 3600))
            ...
            if wandb_logger.wandb and not self.opt.evolve:  # Log the stripped model
    ...
    def run(self):
        ...
        if self.opt.resume and not wandb_run:  # resume an interrupted run
            ...
            self.opt.cfg, self.opt.weights, self.opt.resume, self.opt.batch_size, self.opt.global_rank, self.opt.local_rank = '', ckpt, True, self.opt.total_batch_size, *apriori  # reinstate
            self.logger.info('Resuming training from %s' % ckpt)
        else:
            ...
            self.opt.save_dir = increment_path(Path(self.opt.project) / self.opt.name, exist_ok=self.opt.exist_ok | self.opt.evolve)  # increment run
        ...
        if not self.opt.evolve:
            ...
            if self.opt.global_rank in [-1, 0]:
                ...
                self.logger.info(f"{prefix}Start with 'tensorboard --logdir {self.opt.project}', view at http://localhost:6006/")
                tb_writer = SummaryWriter(self.opt.save_dir)  # Tensorboard
            self.train(hyp, device, tb_writer)
        ...
        # Evolve hyperparameters (optional)
            ...
            for _ in range(300):  # generations to evolve
                ...
                # Train mutation
                results = train(hyp.copy(), self.opt, device)
    ...
    def get_arguments(self):
        ...
        parser.add_argument('--weights', type=str, default=self.weights_path, help='initial weights path')
        parser.add_argument('--cfg', type=str, default=self.cfg, help='model.yaml path')
        parser.add_argument('--data', type=str, default=self.data_yaml, help='data.yaml path')
        parser.add_argument('--hyp', type=str, default=self.hyp, help='hyperparameters path')
        parser.add_argument('--epochs', type=int, default=self.epochs)
        parser.add_argument('--batch-size', type=int, default=self.batch_size, help='total batch size for all GPUs')
        parser.add_argument('--img-size', nargs='+', type=int, default=self.img_size, help='[train, test] image sizes')
        parser.add_argument('--rect', action='store_true', help='rectangular training')
        parser.add_argument('--resume', nargs='?', const=True, default=self.resume, help='resume most recent training')
        parser.add_argument('--nosave', action='store_true', help='only save final checkpoint')
        parser.add_argument('--notest', action='store_true', help='only test final epoch')
        parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check')
        parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters')
        parser.add_argument('--bucket', type=str, default=self.bucket, help='gsutil bucket')
        parser.add_argument('--cache-images', action='store_true', help='cache images for faster training')
        parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')
        parser.add_argument('--device', default= self.devices, help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
        parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')
        parser.add_argument('--single-cls', action='store_true', help='train multi-class data as single-class')
        parser.add_argument('--adam', action='store_true', help='use torch.optim.Adam() optimizer')
        parser.add_argument('--sync-bn', action='store_true', help='use SyncBatchNorm, only available in DDP mode')
        parser.add_argument('--local_rank', type=int, default=-1, help='DDP parameter, do not modify')
        parser.add_argument('--workers', type=int, default=self.num_workers, help='maximum number of dataloader workers')
        parser.add_argument('--project', default='runs/train', help='save to project/name')
        parser.add_argument('--entity', default=None, help='W&B entity')
        parser.add_argument('--name', default=self.name, help='save to project/name')
        ...
        parser.add_argument('--save_period', type=int, default=self.save_period, help='Log model after every "save_period" epoch')
        parser.add_argument('--artifact_alias', type=str, default="latest", help='version of dataset artifact to be used')
        parser.add_argument('--freeze', nargs='+', type=int, default=self.freeze, help='Freeze layers: backbone of yolov7=50, first3=0 1 2')
        parser.add_argument('--v5-metric', action='store_true', help='assume maximum recall as 1.0 in AP calculation')
        self.opt = parser.parse_args()

        self.opt.rect = self.rect
        self.opt.nosave = self.nosave
        self.opt.notest = self.notest
        self.opt.noautoanchor = self.noautoanchor
        self.opt.evolve = self.evolve
        self.opt.cache_images = self.cache_images
        self.opt.image_weights = self.image_weights
        self.opt.multi_scale = self.multi_scale
        self.opt.single_cls = self.single_cls
        self.opt.adam = self.adam
        self.opt.sync_bn = self.sync_bn
        self.opt.entity = self.entity
        self.opt.exist_ok = self.exist_ok
        self.opt.quad = self.quad
        self.opt.linear_lr = self.linear_lr
        self.opt.upload_dataset = self.upload_dataset
        self.opt.artifact_alias = self.artifact_alias
        self.opt.v5_metric = self.v5_metric
